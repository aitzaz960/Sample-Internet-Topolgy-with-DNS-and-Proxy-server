//We are asssuming that our client and server are in same system so we dont need to deal with IPs that much
...
int main()
{
	...
	while(1)
		...
		struct timeval tv={50,0};	//Set timeout of 5 seconds
			...
			for(i=0;i<MAXFD;++i)
			{
				...
				//There are two kinds of cases for judging file descriptors
					...
					if(fds[i]==sockfd)	//A file descriptor is a socket, meaning accept if a new client requests a connection
					{
						...
						temp.my_port = PORT;
						temp.conn_port = (int) ntohs(caddr.sin_port);
						temp.hop = 1;
						temp.path.push_back(temp.my_port);
						temp.path.push_back(temp.conn_port);
						table.push_back(temp);
						temp.path.clear();
						cout << "****************BROADCASTING TABLE******************\n";
						cout << "My port\t\t\tConnected Port\t\t\tHop Count\t\t\tPath\n";
						for(int i=0; i<table.size(); i++)
						{
							cout<<i+1<<". "<<table[i].my_port<<"\t\t\t"<<table[i].conn_port<<"\t\t\t"<<table[i].hop<<"\t\t\t";
							for(int j=0; j<table[i].path.size(); j++)
								cout << table[i].path[j] << "--->" ;
							cout << endl;	
						}		
